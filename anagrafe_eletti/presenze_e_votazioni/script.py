...
conn = sqlite3.connect('PRESENZE_VOTAZIONI.db')
...
for row in conn.execute('select strftime("%Y-%m",Data) AS DATA, COUNT(Presenza) AS TOTALE_PRESENZE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "1" AND NOMINATIVO == "CARELLA DANIELE" GROUP BY Progressivo, strftime("%Y-%m",Data) ORDER BY DATA ASC'):
    date_carella.append(row[0])
    presenze_carella.append(row[1])

# presenze suddivise per mese ERRANI FRANCESCO
for row in conn.execute('select strftime("%Y-%m",Data) AS DATA, COUNT(Presenza) AS TOTALE_PRESENZE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "1" AND NOMINATIVO == "ERRANI FRANCESCO" GROUP BY Progressivo, strftime("%Y-%m",Data) ORDER BY DATA ASC'):
    date_errani.append(row[0])
    presenze_errani.append(row[1])

# presenze suddivise per mese ALDROVANDI STEFANO
for row in conn.execute('select strftime("%Y-%m",Data) AS DATA, COUNT(Presenza) AS TOTALE_PRESENZE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "1" AND NOMINATIVO == "ALDROVANDI STEFANO" GROUP BY Progressivo, strftime("%Y-%m",Data) ORDER BY DATA ASC'):
    date_aldrovandi.append(row[0])
    presenze_aldrovandi.append(row[1])

# presenze suddivise per mese MEROLA VIRGINIO
for row in conn.execute('select strftime("%Y-%m",Data) AS DATA, COUNT(Presenza) AS TOTALE_PRESENZE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "1" AND NOMINATIVO == "MEROLA VIRGINIO" GROUP BY Progressivo, strftime("%Y-%m",Data) ORDER BY DATA ASC'):
...
# presenze totali
for row in conn.execute('select Nominativo,COUNT(Presenza) AS TOTALE from PRESENZE_VOTAZIONI WHERE Presenza = "P" GROUP BY Progressivo ORDER BY TOTALE DESC'):
...
for row in conn.execute('select Nominativo,COUNT(Presenza) AS TOTALE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "5" GROUP BY Progressivo ORDER BY TOTALE DESC'):
...
for row in conn.execute('select Nominativo,COUNT(Presenza) AS TOTALE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) == "1" GROUP BY Progressivo ORDER BY TOTALE DESC'):
...
# altre presenze
for row in conn.execute('select Nominativo,COUNT(Presenza) AS TOTALE from PRESENZE_VOTAZIONI WHERE Presenza = "P" AND strftime("%w",Data) != "1" AND strftime("%w",Data) != "5" GROUP BY Progressivo ORDER BY TOTALE DESC'):
...
# totale votazioni
for row in conn.execute('select Nominativo,SUM(NumVotazioni) AS TOTALE from PRESENZE_VOTAZIONI WHERE Presenza = "P" GROUP BY Progressivo ORDER BY TOTALE DESC'):
